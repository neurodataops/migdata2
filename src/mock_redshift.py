"""
mock_redshift.py — Mock Source System (implements SourceAdapter)
================================================================
Generates a synthetic Redshift catalog and query history using Faker.

Implements src.interfaces.SourceAdapter so a real RedshiftSourceAdapter
can replace it by changing config.yaml: source.adapter: "redshift".

Run standalone:
    python -m src.mock_redshift
    python -m src.mock_redshift --seed 42
"""
...
COLUMN_TEMPLATES = {
    "customer": [
        ...
        ("lifetime_value", "DECIMAL(12,2)", True, False),
    ...
    "order": [
        ...
        ("total_amount", "DECIMAL(14,2)", False, False),
        ("discount_amount", "DECIMAL(10,2)", True, False),
        ("shipping_cost", "DECIMAL(8,2)", True, False),
        ("currency_code", "VARCHAR(3)", False, False),
        ("created_at", "TIMESTAMP", False, False),
        ("warehouse_id", "INTEGER", True, False),
    ],
    "order_detail": [
        ("item_id", "INTEGER", False, True),
        ("order_id", "INTEGER", False, False),
        ("product_id", "INTEGER", False, False),
        ("quantity", "INTEGER", False, False),
        ("unit_price", "DECIMAL(10,2)", False, False),
        ("discount_pct", "DECIMAL(5,2)", True, False),
        ("line_total", "DECIMAL(12,2)", False, False),
        ("created_at", "TIMESTAMP", False, False),
    ],
    "product": [
        ...
        ("cost", "DECIMAL(10,2)", True, False),
        ("weight_kg", "DECIMAL(6,2)", True, False),
        ("is_active", "BOOLEAN", False, False),
        ("created_at", "TIMESTAMP", False, False),
        ("updated_at", "TIMESTAMP", True, False),
    ],
    ...
    "event": [
        ("event_id", "BIGINT", False, True),
        ("user_id", "INTEGER", False, False),
        ("event_type", "VARCHAR(64)", False, False),
        ("event_timestamp", "TIMESTAMP", False, False),
    ...
    "segment": [
        ...
        ("criteria_json", "VARCHAR(4096)", True, False),
        ("user_count", "INTEGER", True, False),
        ("created_at", "TIMESTAMP", False, False),
        ("updated_by", "VARCHAR(64)", True, False),
    ],
    "experiment": [
        ("test_id", "INTEGER", False, True),
        ("test_name", "VARCHAR(128)", False, False),
        ("variant", "VARCHAR(32)", False, False),
        ("user_id", "INTEGER", False, False),
        ("conversion", "BOOLEAN", False, False),
        ("revenue", "DECIMAL(10,2)", True, False),
        ("assigned_at", "TIMESTAMP", False, False),
        ("converted_at", "TIMESTAMP", True, False),
    ],
    "staging": [
        ("row_id", "BIGINT", False, True),
        ("raw_payload", "VARCHAR(65535)", True, False),
    ...
    "finance": [
        ...
        ("amount", "DECIMAL(14,2)", False, False),
        ("currency_code", "VARCHAR(3)", False, False),
    ...
    "campaign": [
        ("campaign_id", "INTEGER", False, True),
        ("campaign_name", "VARCHAR(256)", False, False),
        ("channel", "VARCHAR(64)", False, False),
        ("start_date", "DATE", False, False),
        ("end_date", "DATE", True, False),
        ("budget", "DECIMAL(12,2)", True, False),
        ("spend", "DECIMAL(12,2)", True, False),
        ("status", "VARCHAR(32)", False, False),
        ("created_by", "VARCHAR(64)", True, False),
        ("created_at", "TIMESTAMP", False, False),
    ],
    "conversion": [
        ("conversion_id", "BIGINT", False, True),
        ("user_id", "INTEGER", False, False),
        ("campaign_id", "INTEGER", True, False),
        ("conversion_type", "VARCHAR(64)", False, False),
        ("revenue", "DECIMAL(12,2)", True, False),
        ("converted_at", "TIMESTAMP", False, False),
        ("attribution_model", "VARCHAR(32)", True, False),
    ],
    "attribution": [
        ("touch_id", "BIGINT", False, True),
        ("user_id", "INTEGER", False, False),
        ("campaign_id", "INTEGER", False, False),
        ("channel", "VARCHAR(64)", False, False),
        ("touch_timestamp", "TIMESTAMP", False, False),
        ("touch_position", "INTEGER", False, False),
        ("is_converting_touch", "BOOLEAN", False, False),
        ("credit", "DECIMAL(5,4)", True, False),
    ],
}

# FK relationships (child_schema.child_table.child_col -> parent_schema.parent_table.parent_col)
FK_RELATIONSHIPS = [
...
# Stored procedure templates (Redshift PL/pgSQL style)
PROC_TEMPLATES = [
    {
...
DECLARE
    v_count INT;
BEGIN
    DELETE FROM analytics.user_segments WHERE created_at < DATEADD(day, -90, GETDATE());

    INSERT INTO analytics.user_segments (segment_name, criteria_json, user_count, created_at)
    SELECT 'high_value', '{"ltv_min": 1000}',
           COUNT(*), GETDATE()
    FROM public.customers
    WHERE lifetime_value >= 1000;

    GET DIAGNOSTICS v_count = ROW_COUNT;
    RAISE INFO 'Refreshed segments: % rows', v_count;
END;
$$ LANGUAGE plpgsql;""",
    },
    {
...
BEGIN
    ...
    SELECT 'SNAP-' || TO_CHAR(p_date, 'YYYYMMDD'),
           SUM(total_amount), 'USD', p_date, 'posted', GETDATE()
    FROM public.orders
    WHERE order_date = p_date AND status = 'completed';

    IF NOT FOUND THEN
        RAISE WARNING 'No completed orders for %', p_date;
    END IF;
END;
$$ LANGUAGE plpgsql;""",
    },
...
BEGIN
    FOR rec IN v_cursor LOOP
        RAISE INFO 'Campaign: %, Spend: %, Revenue: %, ROI: %',
            rec.campaign_name, rec.spend, rec.total_revenue,
            CASE WHEN rec.spend > 0
                 THEN ROUND((rec.total_revenue - rec.spend) / rec.spend * 100, 2)
                 ELSE 0 END;
    END LOOP;
END;
$$ LANGUAGE plpgsql;""",
...
AS $$
    SELECT CASE
        WHEN CHARINDEX('@', p_email) > 0
        THEN LEFT(p_email, 1) || '***' || SUBSTRING(p_email, CHARINDEX('@', p_email), LEN(p_email))
        ELSE '***'
    END
$$ LANGUAGE sql;""",
    },
...
AS $$
    SELECT 'FY' || EXTRACT(YEAR FROM DATEADD(month, 3, p_date))
           || '-Q' || CEILING(EXTRACT(MONTH FROM DATEADD(month, 3, p_date)) / 3.0)
$$ LANGUAGE sql;""",
    },
]
...
SELECT c.campaign_id, c.campaign_name, c.channel, c.budget, c.spend,
       ...
       COALESCE(SUM(cv.revenue), 0) AS total_revenue,
       CASE WHEN c.spend > 0
            THEN ROUND((COALESCE(SUM(cv.revenue), 0) - c.spend) / c.spend * 100, 2)
...
# Analytical query templates for query_logs
QUERY_TEMPLATES = [
    # --- 10 frequent analytical queries ---
    {
        "label": "daily_revenue",
        "frequency": "high",
...
GROUP BY source_system;""",
    },
    {
       ...
       CAST(description AS DECIMAL(10,4)) AS rate
...
ORDER BY code;""",
    },
    {
       ...
       ROUND(SUM(CASE WHEN conversion THEN 1 ELSE 0 END)::FLOAT / COUNT(*) * 100, 2) AS conv_rate,
...
ORDER BY test_name, variant;""",
    ...
    # --- 5 complex joins ---
    {
       ...
       LISTAGG(DISTINCT at.channel, ', ') WITHIN GROUP (ORDER BY at.channel) AS channels_touched
FROM public.customers c
LEFT JOIN (
...
SELECT tw.channel, c.campaign_name,
       ...
       SUM(CASE WHEN tw.rn = 1 THEN tw.revenue ELSE 0 END) AS last_touch_revenue
...
ORDER BY balance_due DESC;""",
    ...
    WHERE pv.event_timestamp >= DATEADD(day, -30, GETDATE())
    GROUP BY pv.user_id
)
SELECT CASE
         WHEN total_conversions > 0 THEN 'converted'
         WHEN first_funnel_event IS NOT NULL THEN 'engaged'
         WHEN total_sessions > 1 THEN 'returning'
         ELSE 'bounced'
       END AS user_stage,
       COUNT(*) AS user_count,
...
REDSHIFT_USERS = [
    "etl_service", "analyst_jane", "analyst_bob", "bi_dashboard",
    "data_eng_mike", "marketing_api", "finance_bot", "admin_root",
]
...
def generate_source_catalog(seed: int = 42) -> dict:
    """Build a synthetic source catalog matching the real metadata_extractor schema."""
    fake = Faker()
    Faker.seed(seed)
    random.seed(seed)
    ...
    for schema, blueprints in TABLE_BLUEPRINTS.items():
        for table_name, domain in blueprints:
            ...
            tables.append({
                "schema": schema,
                "table": table_name,
                "diststyle": random.choice(["KEY", "EVEN", "ALL", "AUTO"]),
                "sortkey1": None,
                "size_mb": size_mb,
                "rows_estimate": row_estimate,
                "encoded": random.choice(["Y", "N"]),
                "pct_used": round(random.uniform(0.01, 15.0), 2),
            })
            ...
            for ordinal, (col_name, col_type, nullable, is_pk) in enumerate(col_defs, 1):
                columns.append({
                    ...
                    "encoding": random.choice(["lzo", "zstd", "raw", "bytedict"]),
                    "distkey": col_name if is_pk and random.random() > 0.5 else None,
                    "sortkey_position": 0,
                })
    ...
    catalog = {
        ...
        "cluster": "mock-redshift-cluster.us-east-1.redshift.amazonaws.com",
...
def generate_query_logs(catalog: dict, seed: int = 42) -> list:
    ...
    for qt in QUERY_TEMPLATES:
        ...
        for _ in range(count):
            ...
            user = random.choice(REDSHIFT_USERS)
...
class MockSourceAdapter(SourceAdapter):
    """Mock implementation of SourceAdapter using Faker-generated data."""

    def __init__(self, seed: int = None):
        self.seed = seed if seed is not None else get_seed()
        self.log = get_logger("mock_redshift", "mock_redshift.log")
...
def run(seed: int = 42):
    """Generate all mock source data and write to mock_data/."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate mock Redshift source data")
